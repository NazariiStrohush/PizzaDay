...
Meteor.methods({
	'createOrders': function(ordersItem){
		...
		ordersItem = _.extend(ordersItem, {date: new Date(),  userId: Meteor.userId(), usersId: []});
	...
	'addUserOrder': function(ordersItems, sum, groupId){
		...
		if(Orders.findOne(ordersInfo._id).usersId.length == Groups.findOne(groupId).users.length){
			Order.insert(ordersItem);
			Meteor.call('calculateSummaryCost', groupId);
		} else {
